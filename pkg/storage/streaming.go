...
func (se *StorageEngine) FindAllStream(collName string, filter map[string]interface{}) (<-chan domain.Document, error) {
	// For true streaming, we ignore pagination options entirely
	// This allows for memory-efficient streaming of large datasets
	return se.streamGenerator(collName, filter)
}
