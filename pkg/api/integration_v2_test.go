...
// TestServerV2 represents a test HTTP server for integration testing
type TestServerV2 struct {
	Server  *httptest.Server
	TempDir string
	Storage *v2.StorageEngine
	Handler *Handler
	BaseURL string
}

// NewTestServerV2 creates a new test server with temporary storage
func NewTestServerV2(t *testing.T, storageOptions ...v2.StorageOption) *TestServerV2 {
	tempDir, err := os.MkdirTemp("", "go-db-api-test-*")
	require.NoError(t, err)

	// Default options for testing
	defaultOptions := []v2.StorageOption{
		v2.WithDataDir(tempDir),
		v2.WithWALDir(filepath.Join(tempDir, "wal")),
	}

	// Merge with provided options
	allOptions := append(defaultOptions, storageOptions...)

	storageEngine := v2.NewStorageEngine(allOptions...)
...
func (ts *TestServerV2) Close(t *testing.T) {
	ts.Server.Close()
...
func (ts *TestServerV2) POST(path string, body interface{}) (*http.Response, error) {
...
func (ts *TestServerV2) GET(path string) (*http.Response, error) {
	return http.Get(ts.BaseURL + path)
}

func (ts *TestServerV2) PATCH(path string, body interface{}) (*http.Response, error) {
	jsonData, err := json.Marshal(body)
	if err != nil {
		return nil, err
	}
...
func (ts *TestServerV2) PUT(path string, body interface{}) (*http.Response, error) {
	jsonData, err := json.Marshal(body)
	if err != nil {
		return nil, err
	}
...
func (ts *TestServerV2) DELETE(path string) (*http.Response, error) {
...
// ReadResponseBodyV2 reads and returns the response body as a string
func ReadResponseBodyV2(resp *http.Response) (string, error) {
...
func TestAPI_IntegrationV2_BasicCRUD(t *testing.T) {
	...
	var body string
	var err error
	_ = body // suppress unused variable error
	...
	t.Run("Insert Document", func(t *testing.T) {
		...
		documentID = createdDoc["_id"].(string)
		fmt.Printf("DEBUG: Created document ID: %s\n", documentID)
		...
		// Note: v2 engine uses WAL instead of individual collection files
		// Check that WAL directory exists instead
		walDir := filepath.Join(ts.TempDir, "wal")
	...
	t.Run("Get Document by ID", func(t *testing.T) {
		resp, err := ts.GET(fmt.Sprintf("/collections/users/documents/%s", documentID))
		require.NoError(t, err)
		...
		assert.Equal(t, documentID, result["_id"])
	})
	...
	t.Run("Update Document", func(t *testing.T) {
		...
		resp, err := ts.PATCH(fmt.Sprintf("/collections/users/documents/%s", documentID), updates)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		...
		assert.Equal(t, documentID, updatedDoc["_id"])
	...
	t.Run("Replace Document", func(t *testing.T) {
		...
		resp, err = ts.PUT(fmt.Sprintf("/collections/users/documents/%s", documentID), newDoc)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		...
		assert.Equal(t, documentID, replacedDoc["_id"])              // ID preserved
	...
	t.Run("Find All Documents", func(t *testing.T) {
		...
		// Capture the ID of the new document for later deletion
		body, err := ReadResponseBodyV2(resp)
		require.NoError(t, err)
		var newDoc map[string]interface{}
		err = json.Unmarshal([]byte(body), &newDoc)
		require.NoError(t, err)
		newDocumentID = newDoc["_id"].(string)
		...
		require.True(t, ok)
		assert.Len(t, documents, 2) // Replaced document and new document
	})

	t.Run("Delete Document", func(t *testing.T) {
		resp, err := ts.DELETE(fmt.Sprintf("/collections/users/documents/%s", newDocumentID))
		require.NoError(t, err)
		assert.Equal(t, http.StatusNoContent, resp.StatusCode)

		// Verify document is gone
		resp, err = ts.GET(fmt.Sprintf("/collections/users/documents/%s", newDocumentID))
		require.NoError(t, err)
...
func TestAPI_IntegrationV2_TransactionSaves(t *testing.T) {
	ts := NewTestServerV2(t)
	defer ts.Close(t)

	t.Run("WAL Files Created After Each Operation", func(t *testing.T) {
		walDir := filepath.Join(ts.TempDir, "wal")

		// Initially WAL directory should exist (created on startup)
		assert.DirExists(t, walDir)
		...
		// Capture the actual document ID
		body, err := ReadResponseBodyV2(resp)
		require.NoError(t, err)
		...
		// WAL directory should still exist and contain WAL files
		assert.DirExists(t, walDir)

		// Check that WAL files exist (at least one should be created)
		walFiles, err := filepath.Glob(filepath.Join(walDir, "wal_*.log"))
		require.NoError(t, err)
		assert.Greater(t, len(walFiles), 0, "At least one WAL file should exist")

		// Update document
		resp, err = ts.PATCH("/collections/users/documents/"+docID, map[string]interface{}{"value": 43})
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)

		// WAL directory should still exist
		assert.DirExists(t, walDir)
	})
}

func TestAPI_IntegrationV2_TransactionSavesDisabled(t *testing.T) {
	// Create server with transaction saves disabled
	ts := NewTestServerV2(t, v2.WithDurabilityLevel(v2.DurabilityNone))
	defer ts.Close(t)

	t.Run("WAL Files Created Even With DurabilityNone", func(t *testing.T) {
		walDir := filepath.Join(ts.TempDir, "wal")
		...
		// WAL directory should still exist (WAL is always created for recovery)
		assert.DirExists(t, walDir)

		// Document should be retrievable (in memory)
		// Note: We need to get the actual document ID since v2 generates unique IDs
		body, err := ReadResponseBodyV2(resp)
		require.NoError(t, err)
		var doc map[string]interface{}
		err = json.Unmarshal([]byte(body), &doc)
		...
		resp, err = ts.GET("/collections/users/documents/" + docID)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
	})
}

func TestAPI_IntegrationV2_ErrorHandling(t *testing.T) {
	ts := NewTestServerV2(t)
	defer ts.Close(t)
...
func TestAPI_IntegrationV2_ConcurrentRequests(t *testing.T) {
		...
		for i := 0; i < numReaders; i++ {
			...
			go func(readerID int) {
				...
				for j := 0; j < operationsPerGoroutine; j++ {
					resp, err := ts.GET("/collections/readwrite/documents/" + docID)
					if err != nil {
		...
		for i := 0; i < numWriters; i++ {
			...
			go func(writerID int) {
				...
				for j := 0; j < operationsPerGoroutine; j++ {
					...
					resp, err := ts.PATCH("/collections/readwrite/documents/"+docID, update)
					if err != nil {
...
func TestAPI_IntegrationV2_IndexOperations(t *testing.T) {
	ts := NewTestServerV2(t)
	defer ts.Close(t)
...
func TestAPI_IntegrationV2_Pagination(t *testing.T) {
	ts := NewTestServerV2(t)
	defer ts.Close(t)
...
func TestAPI_IntegrationV2_IndexOptimization(t *testing.T) {
	ts := NewTestServerV2(t)
	defer ts.Close(t)
...
func TestAPI_IntegrationV2_PersistenceAcrossRestart(t *testing.T) {
	...
	t.Run("Complete CRUD Operations Persistence", func(t *testing.T) {
		...
		server1 := NewTestServerV2(t, v2.WithDataDir(tempDir))
		...
		// Insert documents and capture IDs
		resp, err := server1.POST("/collections/users", doc1)
		require.NoError(t, err)
		assert.Equal(t, http.StatusCreated, resp.StatusCode)
		body, err := ReadResponseBodyV2(resp)
		require.NoError(t, err)
		var doc1Resp map[string]interface{}
		err = json.Unmarshal([]byte(body), &doc1Resp)
		require.NoError(t, err)
		doc1ID := doc1Resp["_id"].(string)
		resp.Body.Close()
		...
		body, err = ReadResponseBodyV2(resp)
		require.NoError(t, err)
		var doc2Resp map[string]interface{}
		err = json.Unmarshal([]byte(body), &doc2Resp)
		require.NoError(t, err)
		doc2ID := doc2Resp["_id"].(string)
		resp.Body.Close()
		...
		body, err = ReadResponseBodyV2(resp)
		require.NoError(t, err)
		var doc3Resp map[string]interface{}
		err = json.Unmarshal([]byte(body), &doc3Resp)
		require.NoError(t, err)
		doc3ID := doc3Resp["_id"].(string)
		resp.Body.Close()
		...
		resp, err = server1.PATCH("/collections/users/documents/"+doc1ID, patchUpdate)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		resp.Body.Close()
		...
		resp, err = server1.PUT("/collections/users/documents/"+doc2ID, putUpdate)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		resp.Body.Close()

		// Delete a document
		resp, err = server1.DELETE("/collections/users/documents/" + doc3ID)
		require.NoError(t, err)
		assert.Equal(t, http.StatusNoContent, resp.StatusCode)
		...
		server2 := NewTestServerV2(t, v2.WithDataDir(tempDir))
		...
		resp, err = server2.GET("/collections/users/documents/" + doc1ID)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		resp.Body.Close()
		...
		resp, err = server2.GET("/collections/users/documents/" + doc3ID)
		require.NoError(t, err)
		assert.Equal(t, http.StatusNotFound, resp.StatusCode)
	...
	t.Run("Batch Operations Persistence", func(t *testing.T) {
		...
		// Capture the actual document IDs from the batch insert response
		body, err = ReadResponseBodyV2(resp)
		require.NoError(t, err)
		resp.Body.Close()

		var batchInsertResult map[string]interface{}
		err = json.Unmarshal([]byte(body), &batchInsertResult)
		require.NoError(t, err)

		insertedDocs := batchInsertResult["documents"].([]interface{})
		require.Len(t, insertedDocs, 3)

		// Extract document IDs
		doc1ID := insertedDocs[0].(map[string]interface{})["_id"].(string)
		doc2ID := insertedDocs[1].(map[string]interface{})["_id"].(string)

		// Batch update using actual document IDs
		...
		resp, err = server2.GET("/collections/employees/documents/" + doc1ID)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		resp.Body.Close()
	...
	t.Run("Edge Cases Persistence", func(t *testing.T) {
		...
		// Capture the empty document ID
		body, err = ReadResponseBodyV2(resp)
		require.NoError(t, err)
		resp.Body.Close()

		var emptyResult map[string]interface{}
		err = json.Unmarshal([]byte(body), &emptyResult)
		require.NoError(t, err)

		emptyDocID := emptyResult["_id"].(string)
		...
		// Capture the actual document ID from the insert response
		body, err = ReadResponseBodyV2(resp)
		require.NoError(t, err)
		resp.Body.Close()

		var insertResult map[string]interface{}
		err = json.Unmarshal([]byte(body), &insertResult)
		require.NoError(t, err)

		edgeCaseDocID := insertResult["_id"].(string)

		// Test updating to empty document (PUT)
		emptyUpdate := map[string]interface{}{}
		resp, err = server1.PUT("/collections/edge_cases/documents/"+edgeCaseDocID, emptyUpdate)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		resp.Body.Close()
		...
		// First, trigger collection loading by accessing a document
		resp, err = server2.GET("/collections/empty_test/documents/" + emptyDocID)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		...
		assert.Equal(t, emptyDocID, result["_id"])
		// Should only have _id field

		// Verify empty update persisted (document should only have _id)
		resp, err = server2.GET("/collections/edge_cases/documents/" + edgeCaseDocID)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		...
		assert.Equal(t, edgeCaseDocID, result["_id"])
	...
	t.Run("Multiple Collections Persistence", func(t *testing.T) {
		...
		// Insert into users collection and capture IDs
		var userIDs []string
		for _, user := range users {
			...
			userIDs = append(userIDs, result["_id"].(string))
		}

		// Insert into products collection and capture IDs
		...
		for _, product := range products {
			...
			productIDs = append(productIDs, result["_id"].(string))
		}

		// Create orders using actual user and product IDs
		orders := []map[string]interface{}{
			{"user_id": userIDs[0], "product_id": productIDs[0], "quantity": 2},
			{"user_id": userIDs[1], "product_id": productIDs[1], "quantity": 1},
		}
		...
		resp, err := server2.GET("/collections/users_multi/documents/" + userIDs[0])
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		resp.Body.Close()

		resp, err = server2.GET("/collections/products_multi/documents/" + productIDs[0])
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		resp.Body.Close()

		// For orders, we need to get the order IDs from the find results
		...
		err = json.Unmarshal([]byte(body), &ordersResult)
		require.NoError(t, err)

		orderDocs := ordersResult["documents"].([]interface{})
		require.Len(t, orderDocs, 2)

		order1ID := orderDocs[0].(map[string]interface{})["_id"].(string)

		resp, err = server2.GET("/collections/orders_multi/documents/" + order1ID)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, resp.StatusCode)
		resp.Body.Close()
