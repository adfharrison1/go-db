...
export function setup() {
  ...
  console.log('Setting up V2 optimized stress test...');
  console.log(
    'Note: This test is designed for V2 engine with unique ID format'
  );
  ...
  console.log('V2 optimized stress test setup complete');
  return {};
}

// Global storage for document IDs (shared across VUs)
let documentIds = [];
...
export default function (data) {
  ...
  switch (operation) {
    ...
    case 2:
      ...
      if (insertSuccess && insertResponse.json() && insertResponse.json()._id) {
        // Store the document ID for later use
        documentIds.push(insertResponse.json()._id);
        // Keep only last 1000 IDs to prevent memory issues
        if (documentIds.length > 1000) {
          documentIds = documentIds.slice(-1000);
        }
      }
    ...
    case 3:
      ...
      if (
        batchInsertSuccess &&
        batchInsertResponse.json() &&
        Array.isArray(batchInsertResponse.json())
      ) {
        // Store the document IDs from batch insert
        const insertedDocs = batchInsertResponse.json();
    ...
    case 4:
      // Get by ID operation - use actual document IDs
      if (documentIds.length > 0) {
        const randomIndex = Math.floor(Math.random() * documentIds.length);
        const docId = documentIds[randomIndex];
        ...
        // No documents available yet, just do a find operation instead
        const ageFilter = Math.floor(Math.random() * 50) + 18;
        const findResponse = http.get(
          `${BASE_URL}/collections/${COLLECTION}/find?age=${ageFilter}&limit=20`
        );
    ...
    case 5:
      // Update operation - use actual document IDs
      if (documentIds.length > 0) {
        const randomIndex = Math.floor(Math.random() * documentIds.length);
        const docId = documentIds[randomIndex];
